...
export const readTaskEmployee = async (jwt, user) =>
  axios.get(
    `${url}/employees?filters[users_permissions_user][username][$eq]=${user}`,
    {
      headers: {
        Authorization: "Bearer " + jwt,
      },
...
export const readUser = (jwt, user) =>
  axios.get(`${url}/users?filters[username][$eq]=${user}&populate=%2A}`, {
    headers: {
      Authorization: "Bearer " + jwt,
    },
  });
...
export const readEmployeeTask = (jwt, user) =>
  axios.get(
    `${url}/tasks?filters[employee][users_permissions_user][username]=${user}&populate=%2a`,
    {
      headers: {
        Authorization: "Bearer " + jwt,
      },
